...
const HRLeaveResumptionClosedRequest: FC<HRLClosedRequestProps> = ({}) => {
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Closed Leave Resumption');

      // Generate file name
      const fileName = `Closed_Leave_Resumption_${dayjs().format('YYYY-MM-DD_HH-mm-ss')}`;
  ...
  console.log('HRLClosedRequestData', HRLClosedRequestData);
  ...
  `;
    ...
    const {
      data: HRLClosedRequestDataNew,
      refetch : refetchclosed,
      error : isErrorclosed
    } = useQuery({
      queryKey: ['HRLClosedRequestData', paginationData, user?.loginid1, sql_string],
    ...
    <div className="flex flex-col space-y-2">
      ...
      {intl.formatMessage({ id: 'Print' }) || 'Print'}
        ...
        <MyAgGrid
          ...
          rowHeight={25}
          headerHeight={30}
          rowData={HRLClosedRequestDataNew || []}
      ...
      {isErrorclosed && <Typography color="error">Error loading leave approval data.</Typography>}
...
export default HRLeaveResumptionClosedRequest;

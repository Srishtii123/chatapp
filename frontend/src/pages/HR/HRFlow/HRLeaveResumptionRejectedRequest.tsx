...
const HRLeaveResumptionRejectedRequest: FC<HRLRejectedRequestProps> = ({ }) => {
  const intl = useIntl();
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Rejected Leave Resumption');

      // Generate file name
      const fileName = `Rejected_Leave_Resumption_${dayjs().format('YYYY-MM-DD_HH-mm-ss')}`;
    ...
    <div className="flex flex-col space-y-2">
      ...
      {showFormDialog && (
        <DialogPop
          ...
          title={
            selectedRequestNumber
              ? intl.formatMessage({ id: 'Edit Leave Request' })
              : intl.formatMessage({ id: 'Add Leave Request' })
          }
          width="80%"
        >
          <LeaveResumptionForm

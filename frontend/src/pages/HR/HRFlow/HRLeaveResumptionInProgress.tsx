...
const HRLeaveResumptionInProgress: FC<HRLInProgressProps> = ({}) => {
  const intl = useIntl();
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Leave Resumption In Progress');

      // Generate file name
      const fileName = `Leave_Resumption_In_Progress_${dayjs().format('YYYY-MM-DD_HH-mm-ss')}`;
  ...
  const { data: HRLInProgressData } = useQuery({
...
`;
  ...
  const {
    ...
    error : isErrorinprogress
  } = useQuery({
    queryKey: ['Pg_Leave_flow', paginationData, user?.loginid1, sql_string],
  ...
  return (
    <div className="flex flex-col space-y-2">
      ...
      {isErrorinprogress && <Typography color="error">Error loading leave approval data.</Typography>}
...
export default HRLeaveResumptionInProgress;

...
import { IconButton, Menu, MenuItem, Typography, Snackbar, Alert } from '@mui/material';
...
interface HRLLeaveApprovalPageProps {
  showFormDialog?: boolean;
  selectedRequestNumber?: string | null;
  setSelectedRequestNumber?: (value: string | null) => void;
  setShowFormDialog?: (value: boolean) => void;
}

const HRLLeaveApprovalPageBySupervisor = ({
  showFormDialog,
  selectedRequestNumber,
  setSelectedRequestNumber,
  setShowFormDialog
}: HRLLeaveApprovalPageProps) => {
  const { user } = useAuth();
  ...
  const {
    ...
    select: (data) => ({
      ...data,
      tableData: (data?.tableData as { NEXT_ACTION_BY?: string; FLOW_LEVEL_INITIAL?: string; FLOW_LEVEL_RUNNING?: string }[])?.filter(
        (item) => item.NEXT_ACTION_BY === user?.loginid1 && item.FLOW_LEVEL_INITIAL !== item.FLOW_LEVEL_RUNNING
      )
    })
  });
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      // Generate file name
      const fileName = `Leave_Approvals_${dayjs().format('YYYY-MM-DD_HH-mm-ss')}`;
  ...
  const columnDefs = useMemo<ColDef<TLeaveApproval>[]>(
    () => [
      ...
      {
        headerName: intl.formatMessage({ id: 'Contact Details' }) || 'Contact Details',
        field: 'CONTACT_DETAILS_DURING_LEAVE',
        sortable: false,
        filter: false,
        width: 100,
        minWidth: 150,
  ...
  return (
    <div className="flex flex-col space-y-2">
      <div style={{ position: 'relative' }}>
        ...
        <MyAgGrid
          rowData={HR_Data?.tableData || []}
          columnDefs={columnDefs}
          onGridReady={(params) => {
            console.log('Grid Ready, rowData:', HR_Data?.tableData || []);
            onGridReady(params);
          }}
          height="480px"
          rowHeight={20}
      ...
      {showFormDialog && (
        <DialogPop
          ...
          title={
              intl.formatMessage({ id: 'Approve Leave Request' }) || 'Approve Leave Request'
          }
        ...
        >
          <AddLeaveApprovalForm
            ...
            isEditMode={!!selectedRequestNumber} // Pass this prop
            viewMode={viewMode}
          />
        </DialogPop>
      )}
...
export default HRLLeaveApprovalPageBySupervisor;

...
`;
  ...
  const { data: leaveExists, isFetching: isCheckingLeave } = useQuery({
    queryKey: ['LeaveExists', formData.leave_start_date, formData.leave_end_date, user?.loginid1],
    queryFn: async () => {
      const result = await HrRequestServiceInstance.LeaveExists({
        company_code: String(user?.company_code),
        employee_code: String(user?.loginid1),
        leaveStartDate: String(formData.leave_start_date),
        leaveEndDate: String(formData.leave_end_date),
      });
      return result || null;
    },
    enabled: !!formData?.leave_start_date && !!formData.leave_end_date
  });

  console.log('leaveExits',leaveExists);
  ...
  const handleSave = async (actionType: string) => {
    ...
    const {request_number,request_date, EMPLOYEE_ID, leave_type,leave_start_date,leave_end_date,  leave_days,remarks,  SUPERVISOR_EMPID, DEPT_HEAD_EMPID,
      MANGR_EMPID,resume_work,actual_resume_date,DUTY_RESUME_DATE} = formData;
    ...
    if (actionType === 'SAVEASDRAFT' || actionType === 'SUBMITTED') {
      ...
      // Block submission if leave already exists for this date range
      if ((actionType === 'SAVEASDRAFT' || actionType === 'SUBMITTED') && leaveExists?.success) {
        errors.push(leaveExists.message || intl.formatMessage({ id: 'LeaveAlreadyExists', defaultMessage: 'Leave already exists for the selected date range.' }));
      }
  ...
  const RequestNumberExist = formData.request_number !== '' && formData.request_number !== null && formData.request_number !== undefined;
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            <div className="flex flex-wrap gap-2">
              {(isCreator || CreateMode) && (
                <Button type="button" size="small" endIcon={<FaSave />} onClick={() => handleSave('SAVEASDRAFT')} disabled={disableButtons || RequestNumberExist
                }>
              ...
              {isCreator ? (
                <Button
                  ...
                  disabled={disableButtons || isSubmitDisabled() || !!leaveExists?.success || isCheckingLeave}
                  title={
                    leaveExists?.success
                      ? leaveExists.message

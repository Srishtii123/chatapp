...
const HRLeaveResumptionCancelRequest: FC<HRLCancelRequestProps> = ({ }) => {
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Cancel Leave Resumption');

      // Generate file name
      const fileName = `Cancel_Leave_Resumption_${dayjs().format('YYYY-MM-DD_HH-mm-ss')}`;
...
export default HRLeaveResumptionCancelRequest;

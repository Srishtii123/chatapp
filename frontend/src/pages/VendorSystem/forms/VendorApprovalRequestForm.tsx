...
const VendorApprovalRequestForm: React.FC<VendorDetailsTabProps> = ({
  ac_code,
  isEditMode,
  requestData,
  requestNumber,
  ...
  const handleInvoicePrint = async () => {
    const docNo = vendorRequest?.DOC_NO || requestData?.DOC_NO || selectedRefDoc?.DOC_NO || '';
    const companyCode = String(vendorRequest?.COMPANY_CODE || vendorAccountData?.[0]?.COMPANY_CODE || company_code);
    const loginUser = user?.loginid || '';

    if (!docNo) {
      dispatch(showAlert({ open: true, message: intl.formatMessage({ id: 'DocNoRequiredForPrint' }) || 'Doc No is required for printing', severity: 'warning' }));
      return;
    }

    dispatch(openBackdrop());
    try {
      const res = await VendorSerivceInstance.executeVendorInvoicePrint(companyCode, docNo, loginUser);
      if (res?.success) {
        ...
        dispatch(showAlert({ open: true, message: res?.message || intl.formatMessage({ id: 'InvoicePrintFailed' }) || 'Invoice print failed', severity: 'error' }));
      }
    } catch (error: any) {
      dispatch(showAlert({ open: true, message: error?.message || 'Error while printing invoice', severity: 'error' }));
    } finally {
  ...
  const handleSubmit = async (LAST_ACTION: string) => {
    if (!isEditMode) {
      ...
      if (!vendorRequest?.INVOICE_NUMBER?.trim()) {
        dispatch(showAlert({
          open: true,
          message: intl.formatMessage({ id: 'InvoiceNumberisrequired!' }) || 'Invoice Number is required!',
          severity: 'warning'
        }));
        return;
      }
    ...
    try {
      ...
      console.log('Saving ACCOUNT_DATE:', formattedPayload.ACCOUNT_DATE);
  ...
  const loadVendorRequest = async () => {
    if (requestData) {
      try {
        ...
        if (vendorReq) {
           console.log('Full vendorReq:', vendorReq);
           console.log('ACCOUNT_DATE from DB:', vendorReq.ACCOUNT_DATE);
                ...
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      {/* Account Date - readonly, DB se aayi value DD/MM/YYYY */}
                      {isSpecialUser && (
                         <Grid item xs={12} sm={4}>
    <TextField
      fullWidth
      label={intl.formatMessage({ id: 'AccountDate' }) || 'Account Date'}
      variant="outlined"
      value={
    ...
    />
          ...
          {activeTab === 1 && (
            <Box sx={{ p: 2, maxWidth: '100%' }}>
              <VendorItemDetails
                ...
                refDocNo={requestData?.REF_DOC_NO}
              />
            </Box>
          )}
        </Paper>
        ...
        <Grid item xs={12} display="flex" justifyContent="space-between" alignItems="center" mt={3}>
          ...
          <ButtonGroup variant="outlined" size="medium">
            <Tooltip title={intl.formatMessage({ id: 'PrintInvoice' }) || 'Print Invoice'}>
              <span>
                <Button
                  disabled={!(vendorRequest?.DOC_NO || requestData?.DOC_NO || selectedRefDoc?.DOC_NO)}
        ...
        {previewReportPopup.action.open && (
          <UniversalDialog
            ...
            onClose={() => setPreviewReportPopup((p) => ({ ...p, action: { ...p.action, open: false } }))}
...
export default VendorApprovalRequestForm;
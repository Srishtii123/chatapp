...
const VendorApprovalClose = ({ triggerAddPopup, onAddPopupHandled }: VendorMainPgProps) => {
  const { user } = useAuth();
  const intl = useIntl();
    ...
    if (action === 'export') {
      ...
      if (printWindow) {
        printWindow.document.write(`
          <html>
            <head>
              <title>Vendor In Progress</title>
            ...
            <body>
               <h3>{intl.formatMessage({ id: 'Vendor In Progress Requests' }) || 'Vendor In Progress Requests'}</h3>
              <table>
                <thead>
                  <tr>
  ...
  const columnDefs = useMemo<ColDef[]>(
    () => [
      { field: 'DOC_NO', headerName: 'Doc No', width: 140, sortable: true, filter: true, cellStyle: { fontSize: '0.775rem' } },
      {
        field: 'DOC_DATE',
        headerName: 'Doc Date',
        sortable: true,
  ...
  //   WHERE LAST_ACTION = 'APPROVED' AND COMPANY_CODE = 'BSG'
...
WHERE h.FINAL_APPROVED = 'YES' AND NEXT_ACTION_BY = 'APPROVED'
...
export default VendorApprovalClose;

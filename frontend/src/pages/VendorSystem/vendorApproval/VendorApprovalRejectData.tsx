...
const VendorApprovalRejectData = ({ triggerAddPopup, onAddPopupHandled }: VendorMainPgProps) => {
  const { user } = useAuth();
  const intl = useIntl();
      ...
      if (printWindow) {
        printWindow.document.write(`
          <html>
            ...
            <body>
              <h3>Vendor In Progress Requests</h3>
              <table>
                <thead>
                  <tr>
...
WHERE FUN_CHECK_VENDOR_DOC_NO('${user?.loginid ?? ''}', h.DOC_NO) = 'YES'
...
export default VendorApprovalRejectData;

...
const VendorAcEntry = ({ triggerAddPopup, onAddPopupHandled }: VendorMainPgProps) => {
  const { user } = useAuth();
  const intl = useIntl();
  ...
  const columnDefs = useMemo<ColDef[]>(
    () => [
      {
        field: 'DOC_NO', headerName: intl.formatMessage({ id: 'DocumentNumber' }) || 'Document Number',
        width: 170, sortable: true, filter: true, cellStyle: { fontSize: '0.775rem' }
      },
      {
        field: 'DOC_DATE',
        headerName: intl.formatMessage({ id: 'DocumentDate' }) || 'Document Date',
        sortable: true,
        filter: true,
        width: 150,
  ...
  const sql_string = `
    ...
    WHERE LAST_ACTION = 'APPROVED' AND ACCOUNT_NUMBER IS NULL AND COMPANY_CODE = 'BSG'

...
interface MyTaskPendingRequestTabProps {
  costUser: string | null;
}
interface RoleOption {
  role_name: string;   // corresponds to B.ROLE_NAME
  flow_level: number;  // corresponds to A.FLOW_LEVEL
}
...
const { data: getSentbackrolls } = useQuery({
  queryKey: ['getSentbackrollsPf', app],
  queryFn: async (): Promise<{ tableData: RoleOption[]; count: number }> => {
    if (!app) return { tableData: [], count: 0 };

    const response = await PfSerivceInstance.proc_build_dynamic_sql({
      parameter: "getSentbackrollsPf",
    ...
    // Ensure response is an array of RoleOption
    const tableData: RoleOption[] = Array.isArray(response) ? response : [];
...
// Then useEffect can process the fetched data
useEffect(() => {
  const fetchRoles = () => {
    try {
      let data: RoleOption[] = getSentbackrolls?.tableData || [];

      if (request_number.includes('MAT')) {
        PfSerivceInstance.getSentbackrolls_Mat(app).then((res: RoleOption[]) => {
          processRoles(res);
        });
      } else {
        processRoles(data);
  ...
  const processRoles = (data: RoleOption[]) => {
    if (Array.isArray(data) && data.length > 0) {
      ...
      if (filteredRoles.length > 0) {
        const highestRole = filteredRoles.reduce((max, role) =>
          role.flow_level > max.flow_level ? role : max
        );
  ...
  fetchRoles();
}, [app, flowLevel, getSentbackrolls]);
...
const MyTaskPendingRequestTab: FC<MyTaskPendingRequestTabProps> = ({ costUser }) => {
  //--------------constants----------
  ...
  const columns = useMemo<ColumnDef<TVPurchaserequestheader>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const isPORecord = row.original.request_number?.replace(/\//g, '$')?.includes('PO$');
          const actionButtons: TAvailableActionButtons[] = isPORecord ? ['view', 'cancel', 'sentback'] : ['edit', 'reject', 'sentback'];
  ...
  const handleActions = async (actionType: string, rowOriginal: TVPurchaserequestheader) => {
    ...
    const isPORecord = REQUEST_NUMBER?.replace(/\//g, '$')?.includes('PO$');
...
export default MyTaskPendingRequestTab;

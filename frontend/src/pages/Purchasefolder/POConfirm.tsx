...
const POConfirm: FC<POConfirmProps> = ({ costUser }) => {
  //--------------constants----------
  ...
  const columns = useMemo<ColumnDef<TVPurchaserequestheader>[]>(
    () => [
...

      {
        ...
        cell: ({ row }) => {
          const baseButtons: TAvailableActionButtons[] = ['view'];
          let actionButtons: TAvailableActionButtons[] = [...baseButtons];
  ...
  const {
    ...
    queryFn: () => PfSerivceInstance.getMasters(app, 'po_modify', paginationData, searchData)
  ...
  return (
    <div className="flex flex-col space-y-2">
      <h3>User Level: {gs_userlevel || 'Loading...'}</h3> {/* Display gs_userlevel */}

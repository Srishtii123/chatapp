...
const TaskSkillAppraisalTab: React.FC<Props> = ({
  ...
  const grandTotal = useMemo(() => {
    let t = 0;
    rows.forEach((row) => {
      const r = rowStates[text(row.KPI_CODE)]?.RATING;
      t += r === "" || r == null ? 0 : num(r);
    });
  ...
  return (
    <div style={S.wrapper}>
      <table style={S.table}>
        ...
        <tbody>
          {loading ? (
            <tr><td colSpan={3} style={S.emptyMsg}>Loading skills...</td></tr>
          ) : rows.length === 0 ? (
...
export default TaskSkillAppraisalTab;
...
import {Printer,RotateCcw,FileText,Download, Eye,ChevronDown,ChevronUp,} from "lucide-react";
import { useAuth } from "../../../state/AuthContext";
import { getDynamicLookupaccount, getLookupText, getLookupValue, LookupRow } from "../../../api/lookups";
import {getDnSummaryReportHtml,getDnSummaryReportExcelDownload,} from "../../../api/transactions";
import { getGrnSummaryReportExcelDownload, getGrnSummaryReportHtml } from "../../../api/wms";
...
export default function GrnSummaryReportPage() {
    const { user } = useAuth();
    ...
    const fetchReport = useCallback(async (p: Params) => {
        ...
        newTab.document.write(
            "<title>Grn Summary Report</title><body style='font-family:sans-serif;padding:40px;color:#6b7280;'>Loading report…</body>"
        );

        try {
            const html = await getGrnSummaryReportHtml({
    ...
    const handleExcel = async () => {
        setExporting(true);
        try {
            await getGrnSummaryReportExcelDownload({
                parameter: "WMS_Stock_GRN_Summary_Report",
                loginid:   loginId,
                code1:     companyCode,
            ...
            <div style={{ maxWidth: 1400, margin: "0 auto" }}>
                <div style={{ background: "#fff", border: "0.5px solid #e5e7eb", borderRadius: 12, padding: "8px 12px" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                        ...
                        <span style={{ fontSize: 14, fontWeight: 600, color: "#111827" }}>Grn Summary Report</span>
                        {hasGeneratedReport && (
                            <span style={{
                                fontSize: 10,
                                background: "#d1fae5",
                        ...
                        <MultiSelectDropdown
                            ...
                            loadOptions={() =>
                                getDynamicLookupaccount({
                                    parameter: "WMS_Stock_GRN_SUM_principal",
                                    loginid: loginId,
                                    code1: companyCode,
                                    code2: "", code3: "", code4: "",

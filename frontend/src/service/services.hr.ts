...
import { formatDateMontwise, formatDateOnly } from 'utils/dateFormatter';
...
class HrRequestService {
  ...
  async LeaveDaysCount({
    ...
    async LeaveExists({
      ...
      const response = await axiosServices.get('/api/hr/gm/leavedaysexists',{
        params : {
        company_code : company_code,
        employee_code : employee_code,
        leaveStartDate: formatDateMontwise(leaveStartDate),
        leaveEndDate: formatDateMontwise(leaveEndDate),
        },
      });

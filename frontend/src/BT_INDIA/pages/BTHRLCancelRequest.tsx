...
const BTHRLCancelRequest: FC<HRLCancelRequestProps> = ({}) => {
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      // Generate file name
      const fileName = `Canceled_Leave_Requests_${dayjs().format('YYYY-MM-DD_HH-mm-ss')}`;
  ...
  const {
    ...
    queryKey: ['Pg_leave_flow_cancel', searchData, paginationData],
    queryFn: () => BTHrServiceInstance.getMasters('hr', 'Pg_leave_flow_cancel', paginationData, filterData, user?.loginid1),
...
export default BTHRLCancelRequest;

...
import BTAddLeaveApprovalForm from '../forms/BTAddLeaveApprovalForm';
...
const BTHRLLeaveApprovalPage = ({
  ...
  const {
    ...
    queryFn: () => BTHrServiceInstance.getMasters('hr', 'Pg_Leave_flow', paginationData, filterData, user?.loginid1),
  ...
  const { data: editData, refetch: refetchEditData } = useQuery({
    ...
    queryFn: () =>
      selectedRequestNumber
        ? BTHrServiceInstance.getMasters('hr', 'Leaveflow_request', undefined, undefined, selectedRequestNumber)
...
export default BTHRLLeaveApprovalPage;

...
const NavCollapse = ({ menu, level, parentId, setSelectedItems, selectedItems, setSelectedLevel, selectedLevel }: Props) => {
  ...
  useEffect(() => {
    if (getPathNameList(pathname).includes(menu.title?.toString().toLowerCase() as string)) {
      setSelected(menu.id);
    }
        ...
        <>
          ...
          >
            {menuIcon && (
              ...
              >
                {menu.title === 'MASTERS' ? <ManageAccountsIcon /> : menuIcon}
                {menu.title === 'TRANSACTIONS' ? <AccountBalanceIcon /> : menuIcon}
                {menu.title === 'GM' ? <EmojiEventsIcon /> : menuIcon}
                {menu.title === 'INBOUND' ? <CallReceivedIcon /> : menuIcon}
                {menu.title === 'OUTBOUND' ? <CallMadeIcon /> : menuIcon}
                {menu.title === 'STOCKCRITERIA' ? <InventoryIcon /> : menuIcon}
                {menu.title === 'EMPLOYEE' ? <PersonIcon /> : menuIcon}
                {menu.title === 'AGEING' ? <CalendarMonthIcon /> : menuIcon}
                {menu.title === 'PROFIT-AND-LOSS' ? <TrendingUpIcon /> : menuIcon}
              </ListItemIcon>
            )}
              ...
              <ListItemText
                primary={
                  <Typography variant="body1" color="inherit" sx={{ my: 'auto' }}>
                    {snakeCaseToTitleCase(menu.title as string)}
                  </Typography>
                }
              />

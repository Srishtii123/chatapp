...
// FIX: previously this only checked `cell.getValue()` for each rendered column,
// which reads a cell's `accessorKey` value only. Composite-display columns
// (e.g. "EMPLOYEE_CODE - EMPLOYEE_NAME" rendered from accessorKey "EMPLOYEE_CODE")
// meant fields like EMPLOYEE_NAME were never actually searchable, even though
// they were visible on screen. Now we search every raw field on the row object,
// so any underlying data field is searchable regardless of column accessor setup.
const globalIncludesText: FilterFn<unknown> = (row, _columnId, filterValue) => {
  const search = String(filterValue ?? "").trim().toLowerCase();
  if (!search) return true;
  const original = row.original as Record<string, unknown>;
  return Object.values(original).some((value) => String(value ?? "").toLowerCase().includes(search));
};

// Reads TASK_TOTAL/CHAR_TOTAL (falls back to FINAL_RATING) straight off the row
// data and maps it to the same pastel bands used for the Avg Score badge, so the
// whole row is tinted to match. NOTE: this is a plain average — it can't apply
// HR weightage (isHrDefined) since that config isn't available inside the table
// component. If you need the exact weighted score, keep using the badge in the
// FINAL_RATING column as the source of truth; this is a visual-only approximation.
function getRowRatingColor(original: Record<string, unknown>): string | undefined {
  const taskScore = Number(original.TASK_TOTAL ?? original.task_total ?? 0);
  const charScore = Number(original.CHAR_TOTAL ?? original.char_total ?? 0);
  const avg = (taskScore > 0 || charScore > 0)
    ? Math.round((taskScore + charScore) / 2)
    : Math.round(Number(original.FINAL_RATING || 0));

  if (!avg) return undefined;
  if (avg >= 5) return "#e6f9f0";
  if (avg >= 4) return "#e8f0fe";
  if (avg >= 3) return "#f3e8fe";
  if (avg >= 2) return "#fff4e5";
  if (avg >= 1) return "#fdecea";
  return undefined;
}
          ...
          <TableCell className={rowStyle.cell} colSpan={enhancedColumns.length}><Skeleton /></TableCell>
              ...
              visibleRows.map((row) => (
                <TableRow
                  ...
                  style={{ backgroundColor: getRowRatingColor(row.original as Record<string, unknown>) }}

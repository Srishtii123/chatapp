...
import { TPurchaserequestPf_Al, TPrTermCondition_Al } from 'pages/Purchasefolder_Al/purchaserequestheader_pf-types_Al';
...
interface TermsAndConditionProps {
  Purhasedata: TPurchaserequestPf_Al;
  tabIndex: number;
  TermsConditions: TPrTermCondition_Al[];
  setTermsConditions: React.Dispatch<React.SetStateAction<TPrTermCondition_Al[]>>;
}
...
const TermsAndCondition_Al = ({ Purhasedata, tabIndex, TermsConditions, setTermsConditions }: TermsAndConditionProps) => {
  const [loading, setLoading] = useState(true);
  const [message, setMessage] = useState<string | null>(null);
  const [isFirstLoad, setIsFirstLoad] = useState(true); // Flag to track first-time load

  const removeDuplicateSuppliers = (terms: TPrTermCondition_Al[]) => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response = await PfServiceInstance_Al.getMasters(app, 'ddsupplier');
  ...
  const handleInputChange = (field: keyof TPrTermCondition_Al, value: string, index: number) => {
...
export default TermsAndCondition_Al;

...
//new
import GmPfServiceInstance from 'service/service.purhaseflow';
import ProjwisebudgetAllocRpt from '../ProjwisebudgetAllocRpt';
...
const ProjwisebudgetAllocSelection: React.FC = () => {
  ...
  const { data: projectList } = useQuery<ProjectListResponse>({
    ...
    queryFn: async () => {
      const response = await GmPfServiceInstance.getMasters(app, 'projectmaster');
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Box display="flex" flexDirection="column" gap={2} p={2}>
        ...
        {showReport && reportParams && <ProjwisebudgetAllocRpt reportParams={reportParams} onClose={() => setShowReport(false)} />}
      </Box>
    </LocalizationProvider>
  );
};

export default ProjwisebudgetAllocSelection;

...
const BreadcrumbsComp = ({
  ...
  let tempCustomLocation = useMemo(() => customLocation.split('/').filter((eachItem) => eachItem.length > 0), [customLocation]);
  useEffect(() => {
  ...
  if (main && main.type === 'collapse' && main.breadcrumbs === true) {
    ...
    mainContent = (
      <Typography variant="h5" sx={{ textDecoration: 'none' }} color="gray">
        ...
        >
          <Grid item>
            <MuiBreadcrumbs aria-label="breadcrumb" maxItems={maxItems || 8} separator={separatorIcon}>
              <Typography color="gray" variant="h5" sx={{ textDecoration: 'none' }}>
  ...
  if (item && item.type === 'item') {
    if (item.breadcrumbs !== false) {
      breadcrumbContent = (
        <MainCard
          border={card}
          sx={card === false ? { mb: 0.5, bgcolor: 'transparent', ...sx } : { mb: 0.5, ...sx }}
          {...others}
          content={card}
          shadow="none"
        >
          ...
          >
            ...
            <Grid item>
              <MuiBreadcrumbs aria-label="breadcrumb" maxItems={maxItems || 8} separator={'/'}>
                <Typography color="gray" sx={{ textDecoration: 'none' }}>
                  {icons && <HomeOutlined style={iconSX} />}
                  {icon && !icons && <HomeFilled style={{ ...iconSX, marginRight: 0 }} />}
                  {/* {(!icon || icons) && <FormattedMessage id={tempCustomLocation} />} */}
                  {(!icon || icons) && (
                    <MuiBreadcrumbs aria-label="breadcrumb" maxItems={maxItems || 8} separator={'/'}>
                      {tempCustomLocation.map((eachData, index) => (
                        <>
                          {index === tempCustomLocation.length - 1 ? (
                            <Typography className="w-full text-xs font-semibold text-black ">
                              <FormattedMessage id={snakeCaseToTitleCase(eachData)} />
                            </Typography>
                          ) : index === 0 ? (
                            <Typography
                              component={'nav'}
                              onClick={() => navigate(`${app}/dashboard`)}
                              className="w-full text-xs cursor-pointer hover:text-blue-500 text-black "
                            >
            ...
            {/* {title && titleBottom && (
              <Grid item sx={{ mt: card === false ? 0.25 : 1 }}>
                <Typography variant="h2">
                  <FormattedMessage id={snakeCaseToTitleCase(tempCustomLocation[tempCustomLocation.length - 1])} />
                </Typography>
              </Grid>
            )} */}
          </Grid>

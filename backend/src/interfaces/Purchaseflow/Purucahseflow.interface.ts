...
export interface IMaterialCateogrymaster   {
...
export interface ISupplier {
  ...
  curr_code?: string;
  country_code?: string;
  supp_name: string; // This is NOT NULL in the table, so it's required
  supp_addr1?: string;
  supp_addr2?: string;
  supp_addr3?: string;
  supp_addr4?: string;
  ...
  service_date?: Date;
  supp_acref?: string;
  supp_credit?: number; // In the DB it's an int
  supp_stat?: string;
...
export interface IVProjectmaster {
  ...
  created_by?: string; // Created by user (optional)
   // New fields from VW_BUDGET_CURR_STAT_PROJECTWISE_CONSOLIDATED
   po_amount?: number; // PO amount (optional)
   pr_amount?: number; // PR amount (optional)
   prev_appr_amt?: number; // Previously approved amount (optional)
}
...
export interface IPurchaseOrder extends IBasicPurchaseOrder {
  companyCode: string;
  ref_doc_no: string;
  items: IItemPurcharseOrder[];
}

//customer master interface
export interface IMsPsCustomer {
  cust_code: string;
  cust_name: string;
  cust_add1: string;
  cust_add2: string;

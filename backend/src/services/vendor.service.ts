export class VendorService {
  // Used by auth.controller to check external API for user creation
  static async checkAccountEmployee(email: string): Promise<any[]> {
    // Default stub: no external user found
    return [];
  }

  // Used by scheduler to send header/detail records to external system
  static async insertAcHeader(header: any): Promise<any> {
    console.log('[VendorService] insertAcHeader called (stub)', header && header.DOC_NO);
    return { success: true };
  }

  static async insertAcDetail(detail: any): Promise<any> {
    console.log('[VendorService] insertAcDetail called (stub)', detail && detail.DOC_NO);
    return { success: true };
  }
}

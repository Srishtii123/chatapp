...
export class SupportChatService {
  static async ensureSchema() {
    ...
    await addColumnIfMissing("SUPPORT_TICKET", "DEVELOPER_LOGINID", "VARCHAR2(100)");
    await addColumnIfMissing("SUPPORT_TICKET", "DEVELOPER_NAME", "VARCHAR2(400)");
    await addColumnIfMissing("SUPPORT_TICKET", "DEVELOPER_EMAIL", "VARCHAR2(400)");
    ...
    await createTableIfMissing(
      ...
      `CREATE TABLE ${ROOT_SCHEMA}.SUPPORT_DEVELOPER (
        ...
        LOGINID VARCHAR2(100) NOT NULL UNIQUE,
        USERNAME VARCHAR2(400),
        EMAIL_ID VARCHAR2(400),
        COMPANY_CODE VARCHAR2(20),
        SKILL_TAGS VARCHAR2(500),
  ...
  static async getDevelopers() {
    ...
    const result = await oracleDb.query(
      `SELECT DEVELOPER_ID, LOGINID, USERNAME, EMAIL_ID, COMPANY_CODE, SKILL_TAGS, ACTIVE_FLAG,
         ...
         FROM ${ROOT_SCHEMA}.SUPPORT_DEVELOPER
  ...
  static async upsertDeveloper(input: any, user: UserContext) {
    ...
    const existing = await fetchOne(
      `SELECT LOGINID FROM ${ROOT_SCHEMA}.SUPPORT_DEVELOPER WHERE UPPER(LOGINID) = :loginid`,
      { loginid }
    );
    if (existing) {
      await oracleDb.query(
        `UPDATE ${ROOT_SCHEMA}.SUPPORT_DEVELOPER
            SET USERNAME = :username,
                EMAIL_ID = :email,
                COMPANY_CODE = :companyCode,
      ...
      await oracleDb.query(
        `INSERT INTO ${ROOT_SCHEMA}.SUPPORT_DEVELOPER
          (LOGINID, USERNAME, EMAIL_ID, COMPANY_CODE, SKILL_TAGS, ACTIVE_FLAG, CREATED_BY, CREATED_AT, UPDATED_AT)
         VALUES (:loginid, :username, :email, :companyCode, :skillTags, :activeFlag, :createdBy, SYSDATE, SYSDATE)`,
  ...
  static async getDeveloperTickets(user: UserContext) {
    ...
    const result = await oracleDb.query(
      `SELECT TICKET_ID, COMPANY_CODE, TENANT_ID, REQUESTER_LOGINID, REQUESTER_NAME,
        ...
        ORDER BY CASE NVL(DEV_STATUS, 'ASSIGNED')
                   WHEN 'DONE' THEN 3

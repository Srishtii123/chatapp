...
export const insUpdTtePOrderBulk = async (
  req: Request,
  res: Response
): Promise<void> => {
  console.log("insUpdTtePOrderBulk called-------------");
...
user.loginid ?? null,
  ...
  REJECT_REASON:
    ...
    await connection.execute(
      ...
      {
        p_header: {
          type: "TTE_PORDER_HDR_TAB",
          val: [headerRow]
        },

        p_details: {
          type: "TTE_PORDER_DET_TAB",
          val: detailRows
        }
      },
      {
    ...
    res.status(500).json({
      success: false,
      message: "Purchase Order save failed.",

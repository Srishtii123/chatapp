...
export const getLedgerWithOppositeEntryReport = async (req: Request, res: Response): Promise<void> => {
  let connection;
  try {
    const {
      parameter, loginid,
    ...
    const binds: any = {
      parameter: parameter || "Account_Report_Ledger_Grp1",
    ...
    const grandDrFmt = formatBalance(grandTotalDebit);
    const grandCrFmt = formatBalance(grandTotalCredit);
    tableBodyHtml += `
  <tr class="grand-row">
    <td colspan="5" style="text-align:right"><strong>Grand Total :</strong></td>
    <td class="num" colspan="2"><strong>${grandDrFmt}</strong></td>
    <td class="num"><strong>${grandCrFmt}</strong></td>
    <td></td>
  </tr>`;

    const reportTitle = `Ledger With Opposite Entry Report ${text(code5)} - ${text(code6)}`;
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* col widths — 9 cols, no ac_code col */
...
<div class="page">
  ...
  <div class="footer">Generated by ${generatedBy} &bull; ${reportDate}</div>
    ...
    console.error("Ledger With Opposite Entry Report Error:", error);

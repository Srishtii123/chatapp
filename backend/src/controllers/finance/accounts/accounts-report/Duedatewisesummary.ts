...
export const DuedatewiseSummary = async (req: Request, res: Response): Promise<void> => {
    let connection;
    try {
        const {
            loginid,
        ...
        // Fixed parameter for Due Date wise Summary
        const parameter = "Account_Report_VW_PERIODWISE_DUEDATE_SUMMARY";
      ...
      return value < 0
        ...
        rows.forEach((r) => {
            ...
            if (!l4Map.has(l4Key)) l4Map.set(l4Key, { l4_code: l4Key, l4_description: l4Desc, rows: [] });
        ...
        l4Map.forEach((l4) => {
            tableBodyHtml += `
            <tr class="l4-header">
              <td colspan="12"><strong>${l4.l4_code}&nbsp;&nbsp;${l4.l4_description}</strong></td>
            </tr>`;
...
<head>
  <meta charset="utf-8"/>
  <title>Due Date Wise Summary Report</title>
  <style>
    * { box-sizing:border-box; margin:0; padding:0; }
    ...
    table.report-table td { padding:5px 5px; vertical-align:middle; border-bottom:0.5px solid #f0f0f0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font-size:11px; }
    ...
    .l4-total-row td { border-top:1.5px solid #555; border-bottom:2px solid #555; background:#f3f3f3; padding:5px 5px; }
...
<div class="page">
  ...
  <table class="report-table">
    ...
    <tbody>
      ${tableBodyHtml || `<tr><td colspan="14" style="text-align:center;padding:40px;color:#999;">No records found.</td></tr>`}
    </tbody>
  </table>
  ...
  <div class="powered-by">powered by A W A R E</div>
        ...
        console.error("Due Date Wise Summary Report Error:", error);

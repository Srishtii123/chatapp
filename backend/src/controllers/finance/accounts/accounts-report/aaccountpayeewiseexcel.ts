...
function buildAccountPayeeExcelBuffer(
  rows: any[],
  loginid: string,
  parameter: string,
  code5: string,
  ...
  // Row 2 — Report Title
  tableRows.push([xc(`Account Payee Wise Report ${code5} - ${code6}`, "title"), ...Array(NCOLS - 1).fill(skip)]);
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <cellXfs count="13">
    ...
    <!-- 8: subGroup (light gray bg, dark bold, indented) -->
    <xf numFmtId="0" fontId="8" fillId="6" borderId="1" xfId="0" applyFont="1" applyFill="1" applyBorder="1" applyAlignment="1">
      <alignment vertical="center" indent="1"/>
    </xf>
    <!-- 9: totalRow label (light bg, dark bold, right-aligned) -->
    ...
    <!-- 12: numGrand (right-align, #,##0.000, grand total) -->
    <xf numFmtId="164" fontId="9" fillId="7" borderId="1" xfId="0" applyFont="1" applyFill="1" applyBorder="1" applyAlignment="1" applyNumberFormat="1">
      <alignment horizontal="right" vertical="center"/>
    </xf>
  </cellXfs>
  ...
  const workbookXml =
    ...
    ` xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">` +
    `<sheets><sheet name="Account Payee Wise" sheetId="1" r:id="rId1"/></sheets>` +
    `</workbook>`;
...
<head>
  ...
  <style>
    ...
    .page { width: auto; max-width: 100%; margin: 10px auto; padding: 12px; background: #fff; border-radius: 8px; box-shadow: 0 8px 20px rgba(15,23,42,0.08); box-sizing: border-box; }
    ...
    @media print {
      ...
      .group-header, .sub-group-header, .total-row, .closing-row, .grand-total-row { page-break-inside: avoid; break-inside: avoid; }
    }
  </style>
</head>
<body>
...
// ─── Excel Controller ─────────────────────────────────────────────────────────
export const exportAccountPayeeWiseExcel = async (req: Request, res: Response): Promise<void> => {
  let connection;
  try {
    const { parameter, loginid, code5, code6 } = req.body;
    const { rows, connection: conn } = await fetchRows(req);
    ...
    const buffer = buildAccountPayeeExcelBuffer(
      rows,
      text(loginid) || "ADMIN",
      text(parameter),
      text(code5),
    ...
    res.setHeader("Content-Disposition", `attachment; filename="AccountPayeeWiseReport.xlsx"`);
    res.send(buffer);

  } catch (error: any) {
    console.error("Account Payee Wise Excel Error:", error);

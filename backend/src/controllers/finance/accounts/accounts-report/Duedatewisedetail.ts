...
export const DuedatewiseDetail = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        // Fixed parameter for Due Date wise Detail
      ...
      return value < 0
        ...
        // Age labels — same as INV detail (7 buckets)
        ...
        l4Map.forEach((l4) => {
            tableBodyHtml += `
            <tr class="l4-header">
              <td colspan="13"><strong>${l4.l4_code}&nbsp;&nbsp;${l4.l4_description}</strong></td>
            </tr>`;
                ...
                <tr class="ac-header">
                  <td colspan="13">
                    ...
                    <span class="credit-info">
                      Credit Period : ${ac.credit_period}&nbsp;&nbsp;Credit Limit : ${ac.credit_amount}
                    </span>
                  </td>
                </tr>`;
...
<head>
  <meta charset="utf-8"/>
  <title>Due Date Wise Detail Report</title>
  <style>
    * { box-sizing:border-box; margin:0; padding:0; }
    ...
    table.report-table th { border-top:1.5px solid #333; border-bottom:1.5px solid #333; padding:6px 4px; text-align:left; background:#fff; font-size:10px; text-transform:uppercase; white-space:nowrap; }
    table.report-table td { padding:4px 4px; vertical-align:top; border-bottom:0.5px solid #f0f0f0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font-size:11px; }
    ...
    .ac-header td { background:#fff; border-bottom:1px dashed #ccc; padding:5px 8px; }
    ...
    .grand-total-row td { border-top:2.5px double #333; border-bottom:2.5px double #333; background:#eef4fb; padding:6px 4px; font-size:12px; }
...
<div class="page">
  <div class="report-header">
    <table class="meta-table">
      <tr><td class="meta-label">Title :</td><td><strong>Ageing as on ${text(code6)} (Due date wise)</strong></td></tr>
  ...
  <div class="report-title-strip">
    Ageing as on ${text(code6)} ( Due date wise )
  ...
  <table class="report-table">
    ...
    <tbody>
      ${tableBodyHtml || `<tr><td colspan="13" style="text-align:center;padding:40px;color:#999;">No records found.</td></tr>`}
    </tbody>
  </table>

...
function buildTransactionProductExcelBuffer(
  rows: any[],
  loginid: string,
  code2: string
): Buffer {
  ...
  // ── Report header rows ────────────────────────────────────────────────────
  // Row 1 — Company
  tableRows.push([xc("YOUR COMPANY - FREIGHT SOLUTIONS", "company"), ...Array(NCOLS - 1).fill(skip)]);

  // Row 2 — Title
  tableRows.push([xc("Transaction Report — Grouped on Product", "title"), ...Array(NCOLS - 1).fill(skip)]);
  ...
  prodMap.forEach((prod) => {
    ...
    prod.rows.forEach((r) => {
      tableRows.push([
        ...
        xc(text(r.site_code),           "normal"),
        xc(text(r.location_code),       "normal"),
        xc(text(r.txn_type),            "normal"),
        xc(qtyFmt(r.quantity),          "numData"),
        xc(prod.p_uom,                  "normal"),
        xc(0,                           "numData"),   // least qty — bind if available
        xc(prod.l_uom,                  "normal"),
        xc(qtyFmt(r.pqty_cl_balance),  "numData"),
        xc(prod.p_uom,                  "normal"),
        xc(qtyFmt(r.lqty_cl_balance),  "numData"),
        xc(prod.l_uom,                  "normal"),
        xc(formatDateStr(r.mfg_date),   "normal"),
        xc(formatDateStr(r.exp_date),   "normal"),
        xc(text(r.container_no),        "normal"),
        xc(text(r.order_no),            "normal"),
        xc(text(r.job_no),              "normal"),
        xc(text(r.lot_no),              "normal"),
        xc(text(r.batch_no),            "normal"),
        xc(text(r.cust_code),           "normal"),
      ]);
    });
    ...
    tableRows.push([
      ...
      ...Array(4).fill(skip),                  // A–E merged
      xc(finalPCl,  "closingNum"),             // F
      xc(prod.p_uom,"closing"),                // G
      xc(finalLCl,  "closingNum"),             // H
      xc(prod.l_uom,"closing"),                // I
      ...Array(NCOLS - 9).fill(xc("", "closing")), // J–U
    ]);
  ...
  // ── Styles XML ────────────────────────────────────────────────────────────
  // Font index:  0=default  1=company(white bold 18)  2=title(dark bold 14)
  //              3=section(dark bold 11)  4=tableHead(white bold 11)
  //              5=normal   6=numData   7=prodHeader(dark bold 11)
  //              8=closing(dark bold)   9=closingNum
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  const workbookXml =
    ...
    ` xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">` +
    `<sheets><sheet name="Txn Product Report" sheetId="1" r:id="rId1"/></sheets>` +
    `</workbook>`;
...
export const exportTransactionProductExcel = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const binds: any = {
      ...
      code15: null, code16: null, code17: null,
    ...
    console.log("Transaction Product Excel — Dynamic SQL:", rawSql);
    ...
    const buffer = buildTransactionProductExcelBuffer(rows, text(loginid) || "ADMIN", principalCode);
    ...
    console.error("Transaction Product Excel Error:", error);
